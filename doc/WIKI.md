...
3. **GCC 预处理**：使用编译配置文件中的 gcc 命令进行预处理
   - 处理 `#ifdef`、`#if`、`#include` 等指令
   - 应用 `-D`、`-include`、`-imacros` 选项
   - 移除注释（`gcc -E -P`）
4. **语法容错**：解析器忽略无法识别的 C 语法（函数声明、枚举定义等）
5. **两遍扫描解析**：
   - 第一遍：收集所有顶层结构体和联合体的名称
   - 第二遍：解析字段并检测交叉引用
6. **交叉引用检测**：检查自引用、双向和多向循环引用
7. **结果合并**：合并所有头文件的解析结果
8. **JSON 输出**：生成 JSON 格式的结构描述
9. **日志记录**：将错误记录到 `logs/struct-parser.log`，预处理内容记录到 `logs/preprocessed.log`

## 技术栈

- **Java 26**：Record 类、Switch Expressions、Pattern Matching
- **ANTLR4**：语法解析器生成器（支持语法容错）
- **Maven**：构建工具
- **JUnit 5**：测试框架（122+ 测试用例）
...
A: **完全支持**。通过 GCC 预处理支持所有标准 C 条件编译指令：
- `#ifdef`、`#ifndef`、`#if`、`#elif`、`#else`、`#endif`
- 命令行宏定义：`gcc -Dmacro=value`
- 外部宏文件：`gcc -include macros.h` 或 `gcc -imacros macros.h`

**示例：**
```c
#ifdef FEATURE_A
struct FeatureAConfig { uint8 mode; };
#else
struct FeatureBConfig { uint8 level; };
#endif
```

使用 `gcc -E -P -I. -DFEATURE_A` 将只解析 `FeatureAConfig`。

### Q: 头文件中包含函数声明和枚举定义怎么办？

A: **无需担心**。解析器具有语法容错能力，会自动忽略无法识别的 C 语法：
- 函数声明：`void init(void);` → 忽略
- 枚举定义：`enum Color { RED, GREEN };` → 忽略
- 常量定义：`const int MAX = 100;` → 忽略
- 结构体/联合体：正常解析

这使得您可以直接解析复杂的真实头文件，无需预先清理。

### Q: 日志文件在哪里？

A: 应用程序使用 SLF4J + Logback 记录日志，日志文件位于 `logs/` 目录：
- **`logs/struct-parser.log`**：应用日志，包含错误、警告和信息消息
- **`logs/preprocessed.log`**：预处理后的头文件内容（用于调试）
- **控制台**：实时输出执行进度

**日志级别：**
- **ERROR**：解析错误、GCC 失败、配置问题
- **WARN**：非关键性问题
- **INFO**：文件解析进度、结果摘要
- **DEBUG**：详细处理信息、预处理内容
...
- **完善的测试覆盖**：122+ 单元测试，覆盖各种边界场景

### v1.3 - 规划中
- [x] 语法容错：忽略无法识别的 C 语法
- [x] 条件编译支持：`#ifdef`、`-D`、`-include`、`-imacros`
